function registerItemRoutes(fastify, { db }) {
  ...
  fastify.delete('/api/items/:kode_barang', async (request, reply) => {
    ...
    try {
      // Delete all related transaction details first (cascade)
      await db.run('DELETE FROM t_stok_masuk_detail WHERE barang_kode = ?', [kodeParam]);
      ...
      fastify.log.error(err);
      return reply.code(500).send({ error: 'internal error', detail: err.message });
    }
  });
